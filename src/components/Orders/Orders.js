...
function Orders() {
    ...
    function handleCancel(id) {
        try {
            const userID = JSON.parse(localStorage.getItem("user")).userData
                .userId;
            const orderID = id;
        ...
        <div className='orders'>
            ...
            {userData && userData.userData.customizedBurgers.length > 0 && (
                <div className='orders1'>
                    ...
                    <div className='orders-listed'>
                        {userData.userData.customizedBurgers.map(
                            (burger, index) => (
                                <div className='individual-order1' key={index}>
                                    <div className='description'>
                                        <h3>
                                            {burger.name} ({burger.quantity})
                                        </h3>
                                        <p>₹ {burger.price}</p>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className='cancel-button'
                                        onClick={() =>
                                            handleCancelCustomize(burger._id)
                                        }
                                    >
                                        Cancel Order
                                    </button>
